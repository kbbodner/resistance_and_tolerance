...
clearance.df$tolerance.diff<-"no"
...
inf.tol.with.clearance.list<-main.experimental.setup(arena.size=6, 
                                                 infection.length=180,
                                                 var.of.interest="alpha",
                                                 alpha.2=0.0005*5,mu=10*1/100,num.sample.periods = 0)


inf.tol.with.clearance.df<-do.call(rbind.data.frame, inf.tol.with.clearance.list)
inf.tol.with.clearance.df$unique.id<-inf.tol.with.clearance.df$sim.number
#to make it match with clearance dataset remove arena.size and infection.length.minutes
inf.tol.with.clearance.df<-subset(inf.tol.with.clearance.df,select=-c(arena.size,infection.length.minutes))
names(inf.tol.with.clearance.df)[15]<-"sim.num" #change sim.number to sim.num to match clearance

#create identifier for both datasets
inf.tol.with.clearance.df$tolerance.diff<-"yes"

clearance.vs.tolerance.diff<-rbind(clearance.df,inf.tol.with.clearance.df)
